...
##CARREGANDO OS DADOS NA TABELA ASSUNTO_PROXY
SET @idassuntoproxy = (SELECT max(id_assunto_proxy) FROM assunto_proxy);

INSERT INTO assunto_proxy (SELECT (@idassuntoproxy:=@idassuntoproxy+1), id_assunto FROM assunto where substring(codigo_estruturado, 1, 1)  not in ('0','9') and id_tabela_assuntos = @idtabelaatual);
